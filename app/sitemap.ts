...
type ConstructionRow = { slug: string; updated_at: string; category_id: string | null };
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  for (const category of categories ?? []) {
    ...
    entries.push({
      ...
      lastModified: latestOf(categoryRows.map((r) => r.updated_at)),
      changeFrequency: "weekly",
      priority: 0.6,
    });
  }

  // Un tag par requête (getPublishedConstructionsForTag, la même fonction
  ...
  const tagEntries = await Promise.all(
    (tags ?? []).map(async (tag) => {
      const tagConstructions = await getPublishedConstructionsForTag(supabase, tag.id);
